...
- **Linux, Windows и macOS проверяются**, а не поддерживаются на веру. CI гоняет
  набор на каждой, и свежий клон доходит до здоровой установки везде:
  `git clone` → `npm install` → `npm run setup`, где необязательные шаги
  сообщаются как пропущенные, а не как провал.
