...
репозитории (на проект, их видно в pull request).
...
сессии, выжимают из транскрипта запись сессии в конце и советуют, когда сжимать
контекст. Любая ошибка хука завершается кодом 0, поэтому сломанный хук не
блокирует агента.

Три профиля:

- `minimal` — только guard на команды и файлы плюс запись сессии.
...
`allow_config_edits`, `format_on_edit`, пороги сжатия и ваши собственные правила:
