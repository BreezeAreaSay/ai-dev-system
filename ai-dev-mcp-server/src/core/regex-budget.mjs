 ...
 * budget. Every job gets an answer: a job that runs out of budget (or whose
 * worker died) comes back with `matched: null` and `timed_out: true`, and the
 * jobs behind it are still evaluated — in a fresh worker, because terminating
 * the first one is the only way to stop the match it was stuck in.
 *
 * @param {Array<{ pattern: string, flags?: string, haystack?: string }>} jobs
 * @param {{ budgetMs?: number, maxInput?: number }} [options]
 * @returns {Promise<Array<{ matched: boolean | null, timed_out: boolean, error: string, truncated: boolean }>>}
 */
export async function matchWithBudget(jobs, { budgetMs = DEFAULT_MATCH_BUDGET_MS, maxInput = MAX_MATCH_INPUT } = {}) {
  ...
  while (offset < prepared.length) {
    const { results, timedOutAt, error } = await runBatch(prepared.slice(offset), budgetMs);
...
export async function matchOneWithBudget({ pattern, flags = "", haystack = "", budgetMs, maxInput } = {}) {
  ...
  return answer ?? { matched: null, timed_out: false, error: "the match was never run", truncated: false };
}
