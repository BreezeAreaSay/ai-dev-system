...
test("every agent tool has a file to read, and the table decides which", () => {
  // One row per tool, not one branch: Codex and Zed share AGENTS.md, Gemini has
  // its own, Copilot keeps its under .github, and Windsurf and Cline own whole
  // files of their own.
  const byTarget = Object.fromEntries(INSTRUCTION_SURFACES.map((surface) => [surface.target, surface]));
  assert.deepEqual(Object.keys(byTarget), ["agents-md", "gemini-md", "copilot", "windsurf", "cline"]);
  assert.equal(byTarget["copilot"].path, ".github/copilot-instructions.md");
  for (const surface of INSTRUCTION_SURFACES) {
    assert.ok(surface.tools, `${surface.target} does not say which tools read it`);
    assert.ok(["section", "file"].includes(surface.mode));
  }

  const plan = planRuleFiles({ packIds: [], targets: INSTRUCTION_SURFACES.map((surface) => surface.target) });
  assert.deepEqual(plan.map((file) => file.path), INSTRUCTION_SURFACES.map((surface) => surface.path));
  // A shared file gets a section; a file of the tool's own gets the whole
  // content, and says it was generated.
  const shared = plan.find((file) => file.path === "AGENTS.md");
  const owned = plan.find((file) => file.path === ".clinerules/ai-dev.md");
  assert.equal(shared.section.startsWith("## Engineering Rules"), true);
  assert.equal(shared.content, undefined);
  assert.match(owned.content, /generated by ai-dev-system/);
  assert.match(owned.content, /## Engineering Rules/);
});

test("what is installed follows what the repository and the machine already use", () => {
  const present = (paths) => {
    const set = new Set(paths);
    return (target) => set.has(target);
  };
  const bare = detectInstructionTargets({ projectRoot: "/repo", homeDir: "/home/me", exists: present([]) });
  assert.deepEqual(bare, DEFAULT_RULE_TARGETS.filter((target) => bare.includes(target)));
  assert.equal(bare.includes("copilot"), false, "an empty repository is not given files nobody reads");

  const withGithub = detectInstructionTargets({
    projectRoot: "/repo",
    homeDir: "/home/me",
    exists: present([path.join("/repo", ".github"), path.join("/home/me", ".gemini")])
  });
  assert.equal(withGithub.includes("copilot"), true);
  assert.equal(withGithub.includes("gemini-md"), true, "the tool is installed for this person, so its file is worth writing");
  assert.equal(withGithub.includes("windsurf"), false);

  const windsurf = detectInstructionTargets({
    projectRoot: "/repo",
    homeDir: "",
    exists: present([path.join("/repo", ".windsurf")])
  });
  assert.equal(windsurf.includes("windsurf"), true);
  // The defaults are always in, and the order is the canonical one.
  for (const target of DEFAULT_RULE_TARGETS) assert.equal(windsurf.includes(target), true, target);
  assert.deepEqual(windsurf, RULE_TARGETS.filter((target) => windsurf.includes(target)));
});
