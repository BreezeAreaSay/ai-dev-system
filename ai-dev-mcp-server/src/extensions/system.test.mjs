...
async function createFixture(t) {
  ...
  // The benchmark counts as fresh only while the report's mtime is at or above
  // every input's. Writing the report first left the two a millisecond apart
  // under load, which failed the check and flipped the whole run to "fail".
