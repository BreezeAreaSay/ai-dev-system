...
**Файл: `ai-dev-mcp-server/src/core/rules-library.mjs`** (234 строк)
...
/**
 * Plan every file an installation would write.
 *
 * @param {{ packIds: string[], targets?: string[] }} input
 * @returns {Array<{ target: string, path: string, content?: string, section?: string }>}
 */
export function planRuleFiles({ packIds, targets = RULE_TARGETS }) {
  ...
  if (wanted.has("agents-md")) files.push({ target: "agents-md", path: "AGENTS.md", section: renderAgentsRulesSection(packIds) });
  return files;
}
 ...
 * because they carry the generated marker). AGENTS.md gets its
 * `## Engineering Rules` section replaced in place.
 *
 ...
 */
export async function installProjectRules({ projectRoot, stack = [], projectTypes = [], packs, targets = RULE_TARGETS, overwrite = false, dryRun = false }) {
  ...
  for (const file of plan) {
    ...
    if (file.section) {
      ...
      const next = replaceMarkdownSection(current, AGENTS_SECTION, file.section);
      if (next === current) {
...
**Файл: `ai-dev-mcp-server/src/core/rules-library.test.mjs`** (91 строк)
...
**Файл: `ai-dev-mcp-server/src/extensions/rules.mjs`** (89 строк)
...
**Файл: `ai-dev-mcp-server/src/extensions/rules.test.mjs`** (35 строк)
