...
Порядок по размеру функций в `mcp-stdio.mjs` (строки на сегодня):
...
| 3.3 | `security-scan` (команда + скилл: semgrep, trivy, gitleaks, npm audit, pip-audit) | `run_security_scan(project_path, scanners=auto)`: запуск установленных сканеров с нормализованным отчётом `{ tool, severity, file, line, message }`, результат как проверка `security_scan` в `verify_task` рядом с `change_hygiene`; отсутствие сканера = `skipped` с причиной, не ошибка | 4 | M |
...
| 3.7 | `rules-distill` (правила из кодовой базы) | `distill_project_rules(project_path)`: анализ соглашений (импорты, именование, структура тестов, обработка ошибок) из `project-map` и выборки файлов, результат как черновик `.ai-dev/rules/project.md` со статусом draft; дополняет `install_project_rules`, не заменяет | 3 | M |
| 3.8 | `codemaps` (граф модулей и зависимостей) | Расширить `project-map.md` секцией «Import graph»: модули с наибольшим числом зависимых, циклы, точки входа; ленивый пересчёт при `prepare_project` и по `refresh_project_context` | 3 | M |
...
| 3.12 | `prune`, `config-gc` | `prune_state(project_path, dry_run)`: retire инстинктов ниже 0.3 без наблюдений 90 дней, архив сессий старше N, ротация `usage/events.jsonl`, удаление снимков закрытых задач | 2 | S |
