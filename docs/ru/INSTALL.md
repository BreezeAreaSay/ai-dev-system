...
`--dense` — локальную модель BGE-M3 и построенные по ней векторы (около 600 МБ); ни то ни другое не
...
`npm run setup -- --dense`, — и веса ложатся в `~/.ai-dev`, причём кроме Node ничего не нужно.
В опубликованном образе веса скачать нельзя вовсе, и чтобы получить её там, нужно собрать свой вариант образа и
...
совпадает с его словами. В контейнере работают обе, и раз ONNX-рантайм — обычная npm-зависимость,
он уже в образе по умолчанию: включать нечего. Не хватает одного, и по одной причине.

**Весов в публикуемом образе нет.** Они ваши, а не дистрибутива: образ с ними стал бы
скачиванием в несколько сотен мегабайт для каждого и вопросом о лицензии для того, кто его
публикует. Скачайте их один раз на хосте:
...
**Сколько нужно места.** Весов примерно 600 МБ, и скачивание не начнётся на диске, куда они не
влезают вместе с запасом в 512 МБ: вместо того чтобы забить диск до последнего байта, оно скажет,
сколько нужно и сколько есть. Диск, на который они влезают и почти ничего сверх, работает — но
один раз предупредит, что может быть медленно: запись на почти полный диск и есть обычная причина
еле ползущей загрузки, а снаружи это выглядит в точности как оборвавшаяся связь.

Они лягут в `~/.ai-dev/models/bge-m3-onnx` (путь меняется переменной `BGE_M3_ONNX_DIR`).
Монтируется папка *над* ними — `~/.ai-dev/models`, — только для чтения: контейнер веса читает и
никогда не пишет. Одно монтирование обслуживает оба бэкенда, они лежат рядом: `bge-m3-onnx/` и
`bge-m3/`.

**Legacy-бэкенд на Python** никуда не делся и по-прежнему выключен по умолчанию: встроить torch в
каждый образ значило бы заставить всех платить установкой, нужна она им или нет. Он нужен, только
если вы намеренно хотите старый путь:
...
Оба подключают папку как `/models` — именно туда смотрят `BGE_M3_ONNX_DIR` и `BGE_M3_MODEL_DIR`
внутри образа (`/models/bge-m3-onnx` и `/models/bge-m3`).
...
веса, модель нужна внутри. Это ваша сборка, не дистрибутива, и это четыре строки:
...
Такому образу не нужны ни том, ни переменная: `/models/bge-m3-onnx` уже на месте, а именно туда
смотрит `BGE_M3_ONNX_DIR`. Всё остальное — `network_mode: none`, read-only корень,
...
Что стоит за этой строкой, показывает инструмент `embedding_status`: он говорит, какой бэкенд
выбран и почему. Смотреть нужно `dense_backend`: `backend` — это `onnx` или `python`, `available`
отвечает, может ли он работать, а `reason` — не флаг, а фраза. Внутри `dense_backend.onnx`
`missing` перечисляет файлы, которых нет, а `mismatched` — файлы, которые есть и не совпадают с
пиннутым экспортом: первое значит, что том не подключили, второе — что байты не те. Из исходников
`npm run dense:doctor` проходит то же самое шестью стадиями и называет ту, на которой встал.

Legacy-бэкенд оценивается как и раньше: четыре записи в `availability` должны быть `exists: true` —
`embeddings_python`, `model_dir`, `model_file`, `modules_file`, — и если нет интерпретатора, образ
собран без `INSTALL_BGE_M3=1`. Когда выбран ONNX-бэкенд, эти записи ничего не говорят о том,
работает ли плотный поиск.
